...
def question(question_id):
    ...
    question = db.execute('''select 
                             ...
                             from questions join users as asker on asker.id= questions.asked_by_id 
                             join users as expert on expert.id = questions.expert_id 
                             where questions.id = ?''', [question_id])
    question_result = question.fetchone()
...
def ask():
    ...
    if request.method == 'POST':
        ...
        user = user['id']
        db.execute('insert into questions(question_text, asked_by_id, expert_id) values(?,?,?)',[question, user, expert])
        db.commit()
...
def unanswered():
    ...
    '''question_cur = db.execute('select question_text,asked_by_id from questions where answer_text is null and 
        expert_id = ?', [user['id']])
    ...
    for question in question_result:
        ...
        #print(question['asked_by_id'])
        cur = db.execute('select name from users where id = ?',[ question['asked_by_id'] ])
        name_result = cur.fetchone()
        #print(name)
        question_dict['name'] = name_result['name']
