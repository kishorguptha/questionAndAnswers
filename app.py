...
app = Flask(__name__)
app.config['SECRET_KEY'] = b"'\xb0\xcf\x8f\xe8\xc4\xd5\\\x8c\xdc\x14\xe6\x06\x84y\xc7\xce\xa8bV\xf4\xfe\xb5\xc6"
...
def register():
    ...
    if request.method == 'POST':
        ...
        db.execute('insert into users(name, password, expert, admin) values(%s,%s,%s,%s)',(request.form['name'], \
                    hashed_password, '0', '0', ))
...
def ask():
    ...
    if request.method == 'POST':
        ...
        user = user['id']
        db.execute('insert into questions(question_text, asked_by_id, expert_id) values(%s,%s,%s)',(question, user, expert,))
